...
- [x] **목차(TOC)** — `notion-renderer.tsx`가 렌더링하는 `heading_1`/`heading_2`/`heading_3` 블록을 기반으로 목차 컴포넌트 생성
  ...
  - 스크롤 위치 기반 섹션 하이라이트 (후속 개선, 완료) — 초기 `IntersectionObserver` 방식(`rootMargin`으로 하단 70% 제외)은 마지막 heading 아래 남은 여백이 뷰포트 높이보다 짧으면 교차 상태에 진입하지 못해 하이라이트가 이전 섹션에 머무르는 문제가 있었음. `scroll`/`resize` 이벤트로 각 heading의 `getBoundingClientRect().top`을 직접 비교해 "기준선을 통과한 마지막 heading"을 활성화하는 방식으로 교체하고, 스크롤이 문서 최하단에 도달하면 마지막 heading을 강제 활성화하는 예외 처리 추가. 기준선(`SCROLL_OFFSET_PX`)은 heading의 `scroll-mt-24`(96px)와 동일하게 맞춰 앵커 클릭 직후에도 정확히 활성화되도록 함 (Playwright로 최상단/중간/최하단 스크롤과 TOC 클릭 검증 완료)
...
- [x] **코드 블록 문법 강조** (후속 작업, 완료) — `shiki` 도입, `src/lib/shiki-highlighter.ts`에서 fine-grained bundle(`createBundledHighlighter` + lazy import 맵 방식)로 javascript/typescript/tsx/jsx/python/bash/json/css/html/sql/markdown 11개 언어와 `min-light`(라이트)/`nord`(다크) 테마만 번들링해 크기 최소화
  - Notion `code.language` 필드(80여개 Notion 고유 표기)를 shiki 언어 id로 매핑하는 `NOTION_TO_SHIKI_LANG` 테이블 작성 — Notion은 JSX가 섞인 React 코드도 `"typescript"`/`"javascript"`로만 저장하므로(별도 tsx/jsx 값 없음) `typescript→tsx`, `javascript→jsx` grammar로 매핑해 JSX 문법도 안전하게 처리
  - `codeToHtml`은 로드되지 않은 언어로 호출 시 throw하므로, 매핑 테이블에 없는 언어(`"c++"`, `"plain text"` 등)는 shiki 호출 자체를 하지 않고 즉시 `null` 반환(사전 차단) + 매핑이 있어도 try/catch로 런타임 예외 흡수(2차 방어선) — `link-preview.ts`의 실패 흡수 패턴과 동일
  - `HighlightedCodeBlock`(Server, `src/components/common/code-block.tsx`)을 `Suspense`로 감싸 fallback은 기존 `PlainCodeBlock` — `bookmark-card.tsx`와 동일한 패턴
  - 다크모드 대응: `codeToHtml`의 dual theme(`themes:{light,dark}`) 옵션으로 결과 HTML에 `--shiki-dark` 등 CSS 변수를 인라인 생성, `globals.css`에 `html.dark .shiki` 선택자로 다크 변수 활성화(`next-themes` `.dark` 클래스 구조와 호환), shiki 자체 배경은 투명 처리해 기존 `bg-muted` 톤 유지
  - Playwright로 TypeScript(JSX 섞임)/Python 코드 블록의 실제 색상 렌더링, 매핑 없는 C++ 코드의 plain text 폴백, 라이트/다크 모드 전환 시 대비 유지, 375px 반응형, 콘솔 에러 없음 확인 완료
